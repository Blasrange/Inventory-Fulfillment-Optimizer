...
function parseTextData(
  ...
  internalKeys.forEach(internalKey => {
    const possibleHeaders = columnMapping[internalKey].map(h => h.toLowerCase());
    let foundIndex = -1;
    ...
    if (foundIndex !== -1) {
      headerIndices[internalKey] = foundIndex;
        ...
        throw new Error(`Columna requerida no encontrada. Para "${internalKey}", se esperaba una de: "${columnMapping[internalKey].join('", "')}"`);
    }
  });
  ...
  const data = lines.slice(1).map(line => {
    ...
    return record;
      ...
      return Object.values(record).some(val => val !== '' && val !== 0 && val !== null && val !== undefined);
  });
...
function parseExcelData(
    ...
    internalKeys.forEach(internalKey => {
        ...
        if (foundIndex !== -1) {
            ...
            throw new Error(`Columna requerida no encontrada en Excel. Para "${internalKey}", se esperaba una de: "${columnMapping[internalKey].join('", "')}"`);
        }
    });
