'use server';
/**
 * @fileOverview This file defines the Genkit flow for generating restock suggestions based on min/max levels.
 */
...
export type GenerateLevelsAnalysisInput = z.infer<typeof GenerateLevelsAnalysisInputSchema>;
export type GenerateRestockSuggestionsOutput = z.infer<typeof GenerateRestockSuggestionsOutputSchema>;
export type AnalysisResult = z.infer<typeof AnalysisResultSchema>;


export async function generateLevelsAnalysis(input: GenerateLevelsAnalysisInput): Promise<AnalysisResult> {
...
const levelsAnalysisFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    for (const rule of minMaxData) {
        ...
        if (generalSkuInventory && currentStock < cantidadMinima && generalSkuInventory.totalEnReserva > 0) {
            candidates.push({
                sku: sku,
                descripcion: generalSkuInventory.descripcion,
                amountToRestock: cantidadMaxima - currentStock,
                stockEnPicking: currentStock,
                ubicacionesDeReserva: generalSkuInventory.reserveLocations.filter(loc => loc.disponible > 0),
                lpnDestino: lpn,
             ...
             okProducts.push({
                sku: sku,
                descripcion: generalSkuInventory ? generalSkuInventory.descripcion : 'N/A',
                cantidadVendida: 0,
                cantidadDisponible: currentStock,
                cantidadARestockear: 0,
                ubicacionesSugeridas: [{ lpn, localizacion, cantidad: currentStock, diasFPC: null, fechaVencimiento: null }],
                lpnDestino: lpn,
                localizacionDestino: localizacion,
            });
        }
